...
def maxmin_drqn_selection(config: object, agents: dict, k:int = 1000, episodic_flag: bool = True, sg_flag: bool = True, sg_thresh: int = 0, select_method: str = 'DQN'):
    """
    ...
    authority = CentralAuthority(config, agents, k, episodic_flag=episodic_flag, sg_flag=sg_flag, sg_thresh=sg_thresh, select_method=select_method)
    converge = []
    ...
    while True:
        ...
        select_dict, selected_dict = authority.run(select_dict, selected_dict, sum(converge)>0)
...
class CentralAuthority():
    def __init__(self, config: object, agents: dict[int, object], k: int, episodic_flag: bool = True, sg_flag: bool = False, settlement_prob: float = SETTLEMENT_PROB, sg_thresh: int = 0, select_epsilon_decay: float = 0.999, update_times: int = UPDATE_TIMES, select_method: str = 'DDQN'):
        ...
        self.env = Environment(config) if not self.sg_flag else StochasticGameEnvironment(config, thresh=sg_thresh)
    ...
    def check_state(self):
        ...
        if np.random.rand(1) < self.settlement_prob:
            self.state = -1
    ...
    def select_opponent(self, n, s):
        ...
        m = n
        if np.random.rand(1) > self.agents[n].select_epsilon:
