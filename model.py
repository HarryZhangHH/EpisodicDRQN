...
class A2CLSTMVariant(nn.Module):
    """
    Only use in the multi-agent game, all agents have the same critic and each agent has their own actor Net
    """
    def __init__(self, input_size: int, hidden_size: int, num_layers: int, feature_size: int, output_size: int):
        super(A2CLSTMVariant, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        # feature net
        ...
        # actor net
        self.actor = torch.nn.Sequential(
            nn.Linear(hidden_size*2, hidden_size),  # hidden layer
            nn.BatchNorm1d(hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, output_size),
            torch.nn.Softmax(dim=-1)
